    ...
    <form class="row g-2 align-items-end" method="get">
        ...
        <div class="col-md-2 d-flex gap-2">
            ...
            @can('create', App\Models\Role::class)
                <a class="btn btn-primary flex-fill github-btn" href="{{ route('roles.create') }}"><i class="bi bi-plus-lg"></i></a>
            @endcan
        </div>
    </form>
</div>
        ...
        <table class="table table-hover mb-0 data-table">
            ...
            @forelse ($roles as $role)
                <tr>
                    ...
                    <td class="text-end">
                        <a class="btn btn-outline-secondary btn-sm github-btn-icon" href="{{ route('roles.show', $role) }}" title="Ver"><i class="bi bi-eye"></i></a>
                        @can('update', $role)
                            <a class="btn btn-outline-primary btn-sm github-btn-icon" href="{{ route('roles.edit', $role) }}" title="Editar"><i class="bi bi-pencil"></i></a>
                        @endcan
                        @can('delete', $role)
                            <form class="d-inline" method="post" action="{{ route('roles.destroy', $role) }}" data-confirm="¿Desea eliminar el rol {{ $role->name }}?">
                                @csrf @method('delete')
