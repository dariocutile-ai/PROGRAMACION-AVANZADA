    ...
    <form class="row g-2 align-items-end" method="get">
        ...
        <div class="col-md-2 d-flex gap-2">
            ...
            @can('create', App\Models\Category::class)
                <a class="btn btn-primary flex-fill" href="{{ route('categories.create') }}"><i class="bi bi-plus-lg"></i></a>
            @endcan
        </div>
    </form>
</div>
        ...
        <table class="table table-hover mb-0">
            ...
            @forelse ($categories as $category)
                <tr>
                    ...
                    <td class="text-end">
                        <a class="btn btn-outline-secondary btn-sm btn-icon" href="{{ route('categories.show', $category) }}" title="Ver"><i class="bi bi-eye"></i></a>
                        @can('update', $category)
                            <a class="btn btn-outline-primary btn-sm btn-icon" href="{{ route('categories.edit', $category) }}" title="Editar"><i class="bi bi-pencil"></i></a>
                        @endcan
                        @can('delete', $category)
